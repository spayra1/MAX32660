...
float 	get_temp()
{
    ...
    if(value & 0x0100) {
    	...
    	value |= 0xF800;
    	value = ~value+1;
    	real_value = (float) value * (-1) * LC128_DEGREES_RESOLUTION;
    } else {
    	real_value = (float) value * LC128_DEGREES_RESOLUTION;
    }
